...
import com.hcpt.multileagues.database.binder.ParameterBinder;
import com.hcpt.multileagues.database.mapper.RowMapper;
...
public class PrepareStatement {
	 ...
	 * @return
	 */
	public <T> ArrayList<T> select(String table, String columns, String where,
	...
	public <T> ArrayList<T> select(String table, String columns, String where,
			int limit, RowMapper<T> rowMapper) {
		ArrayList<T> modelList = new ArrayList<T>();
	 ...
	 * @return
	 */
	public boolean insertlist(String sql, ArrayList<Object> objectList,
		...
		if (db != null) {
			...
			try {
				...
				SmartLog.log(TAG, MESSAGE_INSERT_A_LIST_OBJECT_SUCCESSFULLY
						+ MESSAGE_LINE);
			} catch (Exception ex) {
				SmartLog.log(TAG, MESSAGE_ERROR_WHEN_IMPLEMENT_QUERY + " : "
						+ sql);
				ex.printStackTrace();
			} finally {
				db.endTransaction();
	...
	public boolean insert(String sql, Object object,
		...
		if (db != null) {
			...
			try {
				...
				statement.execute();
				success = true;
				Log.e(TAG, MESSAGE_INSERT_AN_OBJECT_SUCCESSFULLY + MESSAGE_LINE);
			} catch (Exception ex) {
				Log.e(TAG, MESSAGE_ERROR_WHEN_IMPLEMENT_QUERY + " : " + sql);
				ex.printStackTrace();
			} finally {
				statement.close();
				closeDatabaseConnection();
	...
	public boolean update(String tableName, String set, String where) {
		if (tableName == null || tableName.equalsIgnoreCase("")
				|| where == null || where.equalsIgnoreCase("")) {
		...
		if (db != null) {
			...
			try {
				...
				statement.execute();
				success = true;
				SmartLog.log(TAG, MESSAGE_UPDATE_AN_OBJECT_SUCCESSFULLY
						+ MESSAGE_LINE);
			} catch (Exception ex) {
				SmartLog.log(TAG,
						MESSAGE_ERROR_WHEN_IMPLEMENT_QUERY + sql.toString());
				ex.printStackTrace();
			} finally {
				statement.close();
				closeDatabaseConnection();
	 ...
	 * @return
	 */
	protected Cursor execSelect(SQLiteDatabase db, String table,
			String columns, String where) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT ");
		if (columns == null || columns.equals("")
				|| columns.trim().equalsIgnoreCase("*")) {
			sql.append("*");
		} else {
	...
	private SQLiteDatabase openDatabaseConnection() {
		// Open database connection
		return (new DatabaseOpenhelper(context, DatabaseConfig.getInstance()))
				.getWritableDatabase();
	}
